...
The previous protocol (DKG) demands that everyone participates, however, many threshold cryptography protocols require some subset 
from the entire group that the secret is shared over to participate. 
In this section we extend Lagrange's interpolation algorithm (see \autoref{lagrange-interpolation}) to work in cryptography.
That is, we use Lagrange's interpolation in the exponents of some group. 

\paragraph{Notations and Reminders}
\begin{remark}
  For our convinicece, we publish only shares of the form: $\{(i,P(i))| i > 0\}$.
\end{remark}

Denote: $i_j\in T\subseteq [n]$,  means index of player $j$.
in order to reconstruct we'll need enough indices such that $|T|=deg(P)+1$.

To evaluate the secret, $\mathcal{L}(0)$, we'll need to compute for each $i_j\in T$:
\[
 \ell_{i_j}(0) = \frac{
...
\]
To construct the secret, we need to compute:
\[\mathcal{L}(0) = \sum_{i_j \in T} {\ell_{i_j}\cdot P(i_j)} \]
