...
AES is a substitution-permutation network. The 128-bit input block is arranged as a $4 \times 4$ matrix of bytes, called the \emph{state}. The key is expanded into a sequence of round keys via a key schedule; AES-128 produces 11 round keys. Before the first round, the state is XOR'd with round key 0 (\emph{AddRoundKey}). Then AES-128 performs 10 rounds, each consisting of four steps.

The output is achieved via applying non-linear, yet reversible transformations to the block of bytes, then it mixes the bytes in a manner that ensures every output bit depends on every input bit after a few rounds, and finally it adds the round key to bind the transformation to the secret key.

% TODO: learn AES in full.
% \begin{enumerate}
%   \item \textbf{SubBytes.} Each byte is replaced by its image under a fixed nonlinear S-box, defined by taking the multiplicative inverse of each byte in $\mathrm{GF}(2^8)$ (the field of 256 elements, where arithmetic is done modulo an irreducible degree-8 polynomial over $\mathrm{GF}(2)$), then applying a fixed affine map. This provides \emph{confusion}: the relationship between key and ciphertext is obscured.
%   \item \textbf{ShiftRows.} Row $i$ of the state is cyclically shifted left by $i$ positions, spreading bytes across columns.
%   \item \textbf{MixColumns.} Each column is treated as a degree-3 polynomial over $\mathrm{GF}(2^8)$ and multiplied by a fixed invertible matrix, diffusing bytes within the column. Together with ShiftRows, this ensures every output byte depends on every input byte after two rounds. The final round omits this step.
%   \item \textbf{AddRoundKey.} The state is XOR'd with the current round key, binding the transformation to the secret key.
% \end{enumerate}
% After 10 rounds the output is the 128-bit ciphertext. The combination of SubBytes (nonlinearity) and ShiftRows+MixColumns (diffusion) produces the \emph{avalanche effect}: flipping a single input bit causes roughly half of the output bits to flip, unpredictably.

\paragraph{Modes of operation}
refers to different methods for applying block ciphers (like AES) to encrypt or decrypt data in a way that handles multiple blocks of information.
...
\begin{figure}[h]
\centering
\begin{tikzpicture}[>=stealth, xscale=1.2, yscale=1.1]
  % Axes
  \draw[->] (-2.5,0) -- (5.8,0) node[right] {$x$};
  \draw[->] (0,-0.5) -- (0,5.0);

  % P(x) = 1.0 + 0.9x - 0.15x^2  (true polynomial, S=1.0)
  % P(1)=1.75, P(2)=2.2, P(3)=2.35, P(4)=2.2, P(5)=1.75
  \draw[thick, blue, domain=0:5, samples=80]
    plot (\x, {1.0 + 0.9*\x - 0.15*\x*\x});

  % Q(x) = 2.5 - 1.35x + 0.6x^2  (alternative, S'=2.5)
  % Q(1)=1.75, Q(2)=2.2 -- same as P(1), P(2)
  \draw[thick, violet, dashed, domain=0:3.1, samples=80]
    plot (\x, {2.5 - 1.35*\x + 0.6*\x*\x});

  % True secret S = P(0) = 1.0
  \filldraw[blue]   (0, 1.0) circle (2.5pt) node[left=4pt] {$S  = P(0) = 1.0$};

  % Ambiguous secret S' = Q(0) = 2.5
  \filldraw[violet] (0, 2.5) circle (2.5pt) node[left=4pt] {$S' = Q(0) = 2.5$};

  % Tick at x=0
  \draw (0, 0.06) -- (0,-0.06) node[below] {$0$};

  % All share points on P (grey)
  \foreach \xi in {1, 2, 3, 4, 5} {
    \pgfmathsetmacro{\yi}{1.0 + 0.9*\xi - 0.15*\xi*\xi}
    \filldraw[gray!50] (\xi,\yi) circle (2pt);
    \draw[densely dotted, gray!60] (\xi,\yi) -- (\xi,0)
      node[below, black] {\small $\xi$};
  }

  % f=2 shares known (insufficient): x=1,2 -- both P and Q pass through these
  \foreach \xi in {1, 2} {
    \pgfmathsetmacro{\yi}{1.0 + 0.9*\xi - 0.15*\xi*\xi}
    \filldraw[orange] (\xi,\yi) circle (3.5pt);
  }

  % (f+1)-th share at x=3 -- lies on P but not Q (Q(3)=3.85 vs P(3)=2.35)
  \filldraw[orange!70!black] (3, 2.35) circle (3.5pt);

  % Show where Q would be at x=3 (wrong value) with a cross
  \draw[violet, thick] (3, 3.85) node {$\times$};
  \draw[densely dotted, violet] (3, 3.85) -- (3, 0);
  \node[violet, right=3pt] at (3, 3.85) {\small $Q(3)$};

  % Legend (top-left, clear of both curves)
  \draw[blue, thick]           (-2.3, 4.6) -- (-1.7, 4.6);
  \node[blue,  right] at (-1.6, 4.6) {\small $P(x)$: true polynomial};
  \draw[violet, thick, dashed] (-2.3, 4.2) -- (-1.7, 4.2);
  \node[violet, right] at (-1.6, 4.2) {\small $Q(x)$: consistent with $f$ shares only};
  \filldraw[orange]          (-2.0, 3.8) circle (3pt);
  \node[right] at (-1.6, 3.8) {\small $f$ shares (insufficient)};
  \filldraw[orange!70!black] (-2.0, 3.4) circle (3pt);
  \node[right] at (-1.6, 3.4) {\small $(f{+}1)$-th share (resolves ambiguity)};
\end{tikzpicture}
\caption{With only $f=2$ shares (orange, at $x=1$ and $x=2$), multiple degree-$f$ polynomials
fit the data: both $P$ and $Q$ pass through those points yet yield different secrets
$S=P(0)=1.0$ and $S'=Q(0)=2.5$.
The third share at $x=3$ lies on $P$ (dark orange) but not on $Q$ (violet cross),
uniquely determining the polynomial and hence the secret.}
\end{figure}
...
\subsection{\texorpdfstring{$f$-out-of-$n$}{f-out-of-n} Protocol}
We assume all nodes hold shares of the same polynomial $P$.
This protocol extends the previous one, requiring only $f$-out-of-$n$ signatures to reconstruct the secret.

Each player holds a share $s_j$ (and a matching public key \(pk_j=g^{s_j}\)), derived from polynomial $P$.
...
Message hiding proceeds identically to before. Reconstruction is adjusted so that only $f+1$ signatures are required, rather than $n$, to recover the secret key $sk'$.
% Given $f+1$ signatures over $H(\gamma)$, each participant can recompute the secret and extract out of $c$ the decryption key $sk'$.

Upon receiving a partial signature \(\sigma_{i}=H\left(\gamma\right)^{s_{i}}\), a participant verifies it against \(pk_i\) and stores \((\sigma_i)^{\ell_i(0)}\).
Once $f+1$ such signatures are collected, the participant computes the combined signature:
\[
...
\]
where $\ell_k(0)$ are the Lagrange coefficients defined above.
Given $\sigma$, anyone can recover the ephemeral secret key $sk'$ needed to decrypt $\gamma=Enc_{pk'}\left(tx\right)$:
\begin{align*}
...
\begin{tikzpicture}[>=stealth,
    box/.style={draw, rounded corners, minimum width=2.9cm, minimum height=0.7cm,
                align=center, font=\small},
    arrow/.style={->, thick}]

  % Dealer
  \node[box, fill=blue!15] (dealer) at (0, 0) {Dealer};
  \node[font=\small, text=blue] at (0, -0.55)
    {broadcasts $(c,\,\gamma,\,pk')$};

  % Participant boxes (absolute coordinates)
  \node[box, fill=orange!20] (p1) at (-3.0, -2.0) {Party $i_1$};
  \node[box, fill=orange!20] (p2) at (-0.3, -2.0) {Party $i_2$};
  \node[box, fill=gray!15]   (p3) at ( 2.4, -2.0) {Party $i_3$};
  \node[font=\small]          at ( 4.0, -2.0) {$\cdots$};

  % Subset label above p1..p2
  \node[font=\small, orange] at (-1.65, -1.35)
    {participating subset $T$,\; $|T|=f+1$};
  \draw[orange, thick] (-4.5, -1.6) -- (1.2, -1.6);

  % Broadcast arrows
  \draw[arrow, blue, dashed] (dealer.south) -- (p1.north);
  \draw[arrow, blue, dashed] (dealer.south) -- (p2.north);
  \draw[arrow, blue, dashed] (dealer.south) -- (p3.north);

  % Partial signature labels
  \node[font=\scriptsize] at (-3.0, -2.65)
    {$\sigma_{i_1}=H(\gamma)^{s_{i_1}}$};
  \node[font=\scriptsize] at (-0.3, -2.65)
    {$\sigma_{i_2}=H(\gamma)^{s_{i_2}}$};
  \node[font=\scriptsize, text=gray] at (2.4, -2.65)
    {(not in $T$)};

  % Combine node
  \node[box, fill=green!15, minimum width=7cm] (combine) at (-1.65, -4.1)
    {Combine ($f+1$ partial sigs)\\[3pt]
     $\sigma = \prod_{k\in T}\sigma_k^{\ell_k(0)} = H(\gamma)^S$};

  \draw[arrow] (p1.south) -- ++(0,-0.75) -| (combine.north);
  \draw[arrow] (p2.south) -- ++(0,-0.75) -| (combine.north);

  % Decrypt node
  \node[box, fill=red!10, minimum width=7cm] (decrypt) at (-1.65, -5.4)
    {Recover $sk'$:\quad $c_R\,/\,e(c_L,\sigma) = sk'$};
  \draw[arrow] (combine) -- (decrypt);

\end{tikzpicture}
\caption{Threshold reconstruction in the $f$-out-of-$n$ protocol.
Each party $i \in T$ publishes a partial signature $\sigma_i = H(\gamma)^{s_i}$.
The $f+1$ partial signatures are combined via Lagrange exponentiation to yield
$\sigma = H(\gamma)^S$, from which $sk'$ is recovered without any party learning $S$ in the clear.}
\end{figure}
