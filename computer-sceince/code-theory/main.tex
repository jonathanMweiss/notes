...
Since every polynomial of degree $k-1$ is uniquely defined by $k$ points,
...
\begin{notice}{Problem: Decode $RS_q(\bar{\alpha},n,k)$ from $e\le \lfloor\frac{n-k}{2}\rfloor$}
...
BW key idea is assuming we can create some special polynomial $E(x)$ called
...
\begin{notice}{Why is it important that $E(x)$ is monic?}
...
\subsection{Another take on the explanation}
...
\paragraph{how to achieve max speed}
Using the Half-GCD~\ref{algorithm:half-GCD}, with NTT based divvision we can keep the runtime fast.
