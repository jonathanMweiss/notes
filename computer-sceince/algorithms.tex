...
where $\omega_n$ and we work on elements in 
some ring $Z_q$ (including the polynomial coefficients). 
...
\paragraph[short]{\bf NTT Over Finite Fields} 
The DFT  over $\C$ uses the primitive $n$-th root of unity $\omega_N=e^{2\pi*i/N}$,
but the NTT uses a primitive $N$-th root of unity in $GF(p)$
(i.e., If $N$ is the \emph{least} positive integer such that $\alpha^N = 1 \mod p$
then $\alpha$ is an $N$th root of unity).


There is an algorithm that can find the $N$th root of unity (one exists if $N$ divides $p-1$),
and then one can apply the FFT algorithm by modifying it to first find the
 $n$th root of unity at the beginning of each recursive step of the algorithm ($\omega_n=\alpha$). 

%  \begin{remark}
%   there are ways to reduce the search for a root of unity in each level
%   see \href{https://galois.readthedocs.io/en/v0.0.24/api/galois.ntt.html#:~:text=The%20Number%2DTheoretic%20Transform%20(NTT)%20is%20a%20specialized%20Discrete,for%200%20%3C%20k%20%3C%20N%20}{notes}
%  \end{remark}

Furthermore, when working over a field $\F_p$ such that $p$ is prime,
 the maximal $N$, the array of coefficients representing a
  polynomial, is $N_{max}=p-1$ (\cite[III. number theoretic transforms]{ntt})
...
and $p_i$'s are distinct primes.


\begin{remark}
  the paper articulates why things work even if $M$ is such a prime factorization.
\end{remark}
...
\red{TODO, understand what it means to calculate such GCD}
...
\subsection*{Finding Roots of Unity}
\red{TODO}
