...
\chapter{Number-Theoretic Algorithms}\label{finite-fields}
...
\paragraph{Using Bezout to Find Inverse}
...
To understand the coefficients update, we can view the Euclidean algorithm steps as matrix transformations.
...
\]
By maintaining the product of these transformation matrices, we effectively track the linear combination coefficients $x$ and $y$ such that $r_i = x \cdot \text{original\_}a + y \cdot \text{original\_}b$.
The code above implements exactly this logic, updating the coefficients $(x_0, x_1)$ and $(y_0, y_1)$ according to the transformation matrix $\begin{pmatrix} 0 & 1 \\ 1 & -q \end{pmatrix}$ at each step.
