...
In our case, we modify technique to find a polynomial $T(X)$ to serve as the inverse of our input. That is, we are looking for the root of $F(T)=1-C(X)*T(X)$ over the ring $\mathbb{F}/(x^k)$.
The formula of the technique follows $x_{i+1}=x_i - \frac{f(x_i)}{f'(x_i)}$.
In terms of $T(X)$, the derivative of $F(T)$ would be $-C(x)$.
