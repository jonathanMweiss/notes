...
\begin{insight}{How I think about ideals}
...
\begin{notice}{primes, irreducibility and roots}
...
We begin by describing a super important theorem. It is used wildly in cryptography, and that's why it is here! 

% \begin{insight}{Ordering of elements}
%     I specifically used it to prove how to find roots of unity, which is used to perform the NTT algorithm, and so much more optimizations.
% \end{insight}

(see \autoref{section:fft}). 
\begin{theorem}[Fermat's little theorem]\label{fermats-little-theorem}
    For every element of a finite field $\mathbb{F}_q$ with $q$ elements, 
    $a^q=a$. 
\end{theorem}

\begin{proof}
    \begin{insight}
        Reminder, coprime numbers are numbers that have no common divisors other than 1 ($gcd(a,b)=1$).
    \end{insight}

    If $a$ is not coprime to $q$: since $q$ is prime it must divide $a$, thus $a=0\mod q$ and $a^q=0=a\mod q$.

    Else, we assume $a$ is coprime. Let $A=\{1,2,\dots,(q-1)\}$ be the set of all remainders (except $0$) mod $q$. 
    Let $B=a\cdot A=\{1 \cdot a, 2\cdot a,\dots,(q-1)\cdot a\}$ be the set of all remainders multiplied by $a$.

    First, $0\notin B$ since $a$ and every element in $A$ are coprime to $q$ (and their multiplication is also coprime since it doesn't divide $q$ and vise versa). Second, $B$ should be the same size of $A$: consider $i,j\in A$,
    if $i\cdot a = j \cdot a \mod q$ (since a is coprime, and $q$ is not dividing it, it isn't $0$, and thus we can cancel it in both sides using division), leading to $i=j \mod q$. That is,  $|B|=|A|$.
    So, we get that $B$ mod $q$ has the same elements as in $A$.

    Now inspect the product of all elements:
    \begin{align}
     (a \cdot 1) \cdot (a \cdot 2) \cdot \dots \cdot (a \cdot (q-1)) = 1 \cdot 2 \cdot \dots \cdot (q-1) \mod q \\
     a^{(q-1)}\cdot (q-1)! = (q-1)! \mod q    
    \end{align}

    Since $(q-1)!$ is a product of coprimes of $q$, it equivalent to $0 (\mod q)$. So we can divide both sides of our equation, and we have: 
    $ a^{q-1} = 1 \mod q$
\end{proof}


\begin{corollary}
    Let $\mathbb{F}$ be a finite field with $q$ elements, and let $x\ne0\in \mathbb{F}$.
    Then, $x^{q-1}=1$.
\end{corollary}


\begin{corollary}[Is part of a field?]
    Let $F$ be a subfield of $E$ with $|F|=q$. Then 
    an element $x\in E$ is in $F$ iff $x^q=x$.
\end{corollary}

\begin{corollary}[Modular Inverse]
    Since $a^{q-1}=1 \mod q$, we can find the inverse of $a$ by computing $a^q-1$.
\end{corollary}


\begin{corollary}[Exponent reduction]
    $a^b=a^{b\mod (q-1)} \mod q$.
\end{corollary}

\begin{proof}
    Assume $b>q$ (otherwise it doesn't matter).
    \begin{align}
    a^b=
    \underbrace{a^{q-1} \cdots a^{q-1}}_{\lfloor\frac{b}{q-1} \rfloor\text{ times}} \cdot a^{b\mod (q-1)} \mod q &= \\
     1\cdots1 \cdot a^{b\mod (q-1)} \mod q = a^{b\mod (q-1)} \mod q.    
    \end{align}
\end{proof}
...
That is, a primitive element is a generator of the multiplicative group of the field.

\begin{definition}
    The order of a non zero element $\alpha$ in a finite field $\mathbb{F}_q$ 
    is the smallest positive integer $m$ such that
    $\alpha^m=1$.
\end{definition}

\begin{corollary}
    The order of a generator is $q-1$ (i.e., otherwise it'll have a cycle that does not cover all elements of $\mathbb{F}_q/{0}$.)
\end{corollary}

How can we find such a generator? Since there are quite a lot of them, actually, we can pick a number randmoly and check whether it is a generator or not.


\begin{theorem}
    The number of generators in a field $F_q$ is $\phi(q-1)$ (where $\phi(n)$ is the Euler totient function).
\end{theorem}\label{theorem:num-generators}

(without proof).

\begin{notice} {Euler's Totient Function}
    The \textbf{Euler totient function} $\phi(n)$ counts the number of integers in $\{1, 2, \dots, n\}$ that are coprime to $n$, i.e.,
    \[
        \phi(n) = \bigl|\{k \in \mathbb{Z} : 1 \le k \le n,\ \gcd(k, n) = 1\}\bigr|.
    \]
    For a prime $p$, every integer from $1$ to $p-1$ is coprime to $p$, so $\phi(p) = p - 1$.
    More generally, for $n = p_1^{e_1} \cdots p_r^{e_r}$,
    \[
        \phi(n) = n \prod_{i=1}^{r} \left(1 - \frac{1}{p_i}\right).
    \]
\end{notice}

Our last theorem for this subject is Lagrange's theorem. it is the modified version, specified for us, not for group theory.

\begin{theorem}[Lagrange Group Theorem]\label{theorem:lagrange-groups}
    The \emph{order} of any element, divides the order of the group.
\end{theorem}
...
\begin{notice}{Ordering of elements}
